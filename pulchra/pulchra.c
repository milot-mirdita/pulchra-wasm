...
int read_pdb_file(char *buffer, mol_type *molecules, char *realname) {
  char atmname[10];
  char resname[10];
  char version;
      ...
      if (!_BB_REARRANGE) {
        ...
        while (atom) {
          if (!(atom->name[0] == 'D' && atom->name[1] == 'U') &&
           ...
           out = sdscatprintf(out, "ATOM  %5d  %-3s %3s %c%4d    %8.3f%8.3f%8.3f\n",
...
//    snprintf(out_name, 1000, "%s.tra", "tmp");
